...
if (
    process.env.XSMMURL &&
    process.env.XSMMURL.indexOf(COOKIES_SPLIT) > -1
  ) {
    xsmmurl = process.env.XSMMURL.split(COOKIES_SPLIT);
  } else {
    xsmmurl = process.env.XSMMURL.split();
  }
  if (
    process.env.XSMMHD &&
    process.env.XSMMHD.indexOf(COOKIES_SPLIT) > -1
  ) {
    xsmmhd = process.env.XSMMHD.split(COOKIES_SPLIT);
  } else {
    xsmmhd = process.env.XSMMHD.split();
  }
